(ns wisp.reader
  ...
  (:require [wisp.core.nil :refer [nil?]]
            [wisp.core.boolean :refer [boolean?]]
            [wisp.core.number :refer [number?]]
            [wisp.core.string :refer [string? str subs split join char]]
            [wisp.core.symbol :refer [symbol symbol? gensym]]
            [wisp.core.keyword :refer [keyword keyword?]]
            [wisp.core.re-pattern :refer [re-pattern re-find re-matches re-pattern?]]
            [wisp.core.list :refer [list list? cons]]
            [wisp.core.vector :refer [vector vector? vec]]
            [wisp.core.map :refer [map map? keys vals]]
            [wisp.core.meta :refer [meta with-meta]]
            [wisp.core.named :refer [name namespace]]
            [wisp.core.arithmetic :refer [> < >= <= + - inc dec odd? even?]]
            [wisp.core.sequence :refer [map concat last butlast sort reduce]]
            [wisp.core.equal :refer [=]]
            [wisp.core.sequential :refer [first second rest]]
            [wisp.core.counted :refer [count]]
            [wisp.core.indexed :refer [nth]]
            [wisp.core.conjoinable :refer [conj]]))
  ...
  (loop [buffer ""
    ...
    (cond
     ...
     (identical? \\ ch) (recur (str buffer (escape-char buffer reader))
                               (read-char reader))
     (identical? "\"" ch) (list 'quote buffer)
     :default (recur (str buffer ch) (read-char reader)))))

(defn read-character
  [reader]
  (list 'quote (read-char reader)))
